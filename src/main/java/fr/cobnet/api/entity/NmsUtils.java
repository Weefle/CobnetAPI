...
public class NmsUtils {
	...
	public static final String NMS_PACKAGE_NAME;

	static 
	{
		String tv = CobnetCore.getInstance().getServer().getVersion();
		int index = tv.indexOf("(MC: ") + 5;
		int end = tv.indexOf(")", index);
		String version = tv.substring(index, end);
		NMS_PACKAGE_NAME = "net.minecraft.server.v" + version.replace(".", "_");
	}
	
	/**
	 * Permet d'obtenir une classe nms depuis son nom
	 * @param name
	 * @return
	 */
	public static Class<?> getFromName(String name) {
		try {
			return Class.forName(NMS_PACKAGE_NAME + "." + name);
		}
		
		catch(Exception e) {
			System.err.println("Classe inexistante : " + name);
			return null;
		}
	}
	...
	/**
	 * Fonctionne ?
     * Permet d'enregistrer une entité sans passer par le vieil Enum de l'autre classe également connue
	 * @param name
	 * @param id
	 * @param nmsClass
	 * @param customClass
